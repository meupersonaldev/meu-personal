...
router.get('/', async (req, res) => {
  try {
    ...
    if (!academyId && !franqueadora_id && !userId) {
      return res
        .status(400)
        .json({ error: 'academy_id, franqueadora_id ou user_id obrigatorio' })
    }
    ...
    const limitN = parseInt(limit as string) || 20
    ...
    // Calcular nextCursor
    const nextCursor =
      notifications && notifications.length === limitN
        ? (offset + limitN).toString()
        : null
    ...
    res.json({
      items: notifications || [],
      notifications: notifications || [], // Manter para compatibilidade por enquanto
...
router.patch('/mark-all-read', async (req, res) => {
  try {
    const body = req.body || {}
    const academyId =
      body.academy_id || (req.query.academy_id as string | undefined)
    const franqueadoraId =
      body.franqueadora_id || (req.query.franqueadora_id as string | undefined)
    if (!academyId && !franqueadoraId) {
      return res
        .status(400)
        .json({ error: 'academy_id ou franqueadora_id obrigatorio' })
    }
    let query = supabase
      .from('notifications')
      .update({ read: true, updated_at: new Date().toISOString() })
    ...
    if (academyId) {
      ...
      if (ids.length > 0) query = query.in('academy_id', ids)
        ...
        return res.json({
          message: 'Nenhuma academia encontrada; nada a marcar'
        })
    }
...
router.post('/', async (req, res) => {
  try {
    const notificationSchema = z
      ...
      .refine(data => data.academy_id || data.user_id, {
        message: 'academy_id ou user_id é obrigatório'
      })
    ...
    try {
      if (notification.academy_id) {
        publish(topicForAcademy(notification.academy_id), {
          event: 'notification',
          notification
        })
        ...
        if (academy?.franqueadora_id)
          publish(topicForFranqueadora(academy.franqueadora_id), {
            event: 'notification',
            notification
          })
      }
      if ((notification as any).user_id) {
        publish(topicForUser((notification as any).user_id), {
          event: 'notification',
          notification
        })
      }
    ...
    if (err instanceof z.ZodError) {
      return res
        .status(400)
        .json({ error: 'Dados invalidos', details: err.errors })
    }
...
export async function createNotification (
  ...
  try {
    ...
    try {
      ...
      if (inserted) {
        publish(topicForAcademy(academy_id), {
          event: 'notification',
...
router.get('/stream', async (req, res) => {
  try {
    const { academy_id, user_id, franqueadora_id, since, token } =
      req.query as any
    ...
    if (
      ...
      !req.headers.cookie?.includes('auth-token=')
    ) {
    ...
    return requireAuth(req, res, async () => {
      ...
      if (academy_id)
        unsubs.push(subscribe(topicForAcademy(academy_id), onMessage))
      if (user_id) {
        ...
        if (requestedUserId === currentUserId || isAdmin) {
          ...
          console.warn(
            `Acesso negado ao stream de notificações. Usuário ${currentUserId} tentando acessar notificações de ${requestedUserId}`
          )
        }
      }
      if (franqueadora_id) {
        ...
        if (allowed)
          unsubs.push(
            subscribe(topicForFranqueadora(franqueadora_id), onMessage)
          )
      }
