    ...
    try {
      ...
      if (isCpf) {
        if (!academy?.birth_date || academy.birth_date.trim() === '') {
          return res.status(400).json({
            ...
            message:
              'Data de nascimento é obrigatória para pessoa física (CPF).'
          })
        }
      ...
      if (isCnpj) {
        if (!academy?.company_type || academy.company_type.trim() === '') {
          return res.status(400).json({
            ...
            message:
              'Tipo de empresa é obrigatório para pessoa jurídica (CNPJ).'
          })
        }
        ...
        if (!['MEI', 'LIMITED', 'ASSOCIATION'].includes(academy.company_type)) {
          return res.status(400).json({
            ...
            message:
              'Tipo de empresa inválido. Para CNPJ, deve ser: MEI, LIMITED ou ASSOCIATION.'
          })
        }
      }
      ...
      if (!academy?.monthly_revenue || academy.monthly_revenue <= 0) {
        return res.status(400).json({
          ...
          message:
            'Receita mensal é obrigatória e deve ser maior que zero para criar conta Asaas.'
        })
      }
      ...
      const { data: franqueadora } = await supabase
        .from('franqueadora')
        .select(
          'id, name, email, cnpj, phone, address, city, state, zip_code, asaas_wallet_id'
        )
      ...
      if (!franchisorWalletId) {
        const franchisorWallet = await asaasService.getFranchisorWalletId(
          franqueadora.id
        )
        if (!franchisorWallet.success || !franchisorWallet.walletId) {
          return res.status(500).json({
            ...
            message:
              'Não foi possível obter o walletId da franqueadora. A franqueadora deve ter uma conta Asaas configurada.'
          })
        }
      ...
      if (!accountResult.success || !accountResult.data) {
        console.error(
          '[FRANCHISES] ❌ Erro ao criar conta Asaas:',
          accountResult.error
        )
        ...
        return res.status(500).json({
          ...
          message:
            accountResult.error ||
            'Erro ao criar conta no Asaas. Não foi possível criar a franquia.'
        })
      }
      ...
      if (!asaasAccountId || !asaasWalletId) {
        console.error(
          '[FRANCHISES] ❌ Conta Asaas criada mas sem accountId ou walletId:',
          accountResult.data
        )
        // Rollback: deletar usuário
        await supabase.from('users').delete().eq('id', user.id)
        return res.status(500).json({
          ...
          message:
            'Conta Asaas criada mas sem accountId ou walletId. Não foi possível criar a franquia.'
        })
      }
      ...
      if (franchiseAdminError) {
        console.error(
          'Error creating franchise admin link:',
          franchiseAdminError
        )
...
router.get(
  '/:id',
  requireAuth,
  requireRole(['FRANQUEADORA', 'FRANQUIA', 'SUPER_ADMIN', 'ADMIN']),
  async (req, res) => {
    try {
      const { id } = req.params
...
router.put(
  '/:id',
  requireAuth,
  requireRole(['FRANQUEADORA', 'SUPER_ADMIN', 'ADMIN', 'FRANQUIA']),
  async (req, res) => {
    try {
      const { id } = req.params
      ...
      console.log(
        '[PUT /api/franchises/:id] Atualizando franquia:',
        id,
        updates
      )
      ...
      try {
        if (
          ...
          for (const f of policyFields) {
            if (
              (updates as any)[f] !== undefined &&
              beforePolicy &&
              data &&
              (beforePolicy as any)[f] !== (data as any)[f]
            ) {
              changed[f] = {
                old: (beforePolicy as any)[f],
                new: (data as any)[f]
              }
            }
          }
...
router.delete(
  ...
  async (req, res) => {
    try {
      ...
      await supabase.from('franchise_admins').delete().eq('academy_id', id)

      // 3. Deletar academy_teachers
      await supabase.from('academy_teachers').delete().eq('academy_id', id)

      // 4. Deletar academy_students
      await supabase.from('academy_students').delete().eq('academy_id', id)

      // 5. Deletar academy_plans
      await supabase.from('academy_plans').delete().eq('academy_id', id)

      // 6. Deletar academy_time_slots
      await supabase.from('academy_time_slots').delete().eq('academy_id', id)
      ...
      if (admins && admins.length > 0) {
        for (const admin of admins) {
          ...
          if (!otherAdminRoles || otherAdminRoles.length === 0) {
            await supabase.from('users').delete().eq('id', admin.user_id)
          }
        }
      }
