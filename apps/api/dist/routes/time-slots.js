...
router.get('/', async (req, res) => {
    try {
        ...
        const { data: slots, error } = await supabase_1.supabase
...
router.post('/', async (req, res) => {
    try {
        ...
        const { data: newSlots, error } = await supabase_1.supabase
            .from('academy_time_slots')
...
router.put('/:id', async (req, res) => {
    try {
        ...
        const { data: slot, error } = await supabase_1.supabase
            .from('academy_time_slots')
            .update({
            is_available,
            max_capacity,
    ...
    try {
        ...
        for (const dayConfig of schedule) {
            if (dayConfig.isOpen) {
                ...
                const [openHour, openMinute] = openTime.split(':').map((v) => parseInt(v, 10));
                const [closeHour, closeMinute] = closeTime.split(':').map((v) => parseInt(v, 10));
...
//# sourceMappingURL=time-slots.js.map